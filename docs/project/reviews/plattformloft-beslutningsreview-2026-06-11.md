...
PR #159 er etter seneste status-sync fortsatt draft, mergeable og grønn på GitHub CI. PR #159 er live kilde for siste head/check-status. PR #182 er landet på `main`; JT uke 25-pakken har egen QA-guard i `tests/lib/jt-uke25-package.test.ts`. PR #175 er landet på `main` og gjør prod-data-import-workflowen eksplisitt for `verify-only`, `ownership`, `registers` og `full`, men den kjører ikke prod-import i seg selv. PR #177 er landet på `main` og gjør A4-verifikasjonen kjørbar som `npm run verify:platform-stack-main`. PR #179 er landet på `main`; read-only prod-baseline viser at Coolify SHA Sync er grønn og at live `/api/version` er fasit for gjeldende deploy-SHA. PR #185 er landet på `main` og flytter førsteparters workflows til Node 24-kompatible GitHub Actions-pins (`actions/checkout@v6`, `actions/setup-node@v6`). prod-data-import `verify-only` er kjørt på `main` i GitHub Actions run 27369814363 etter PR #185: tunnelproben rapporterte `Authenticated DB connection ready`, `db:verify` returnerte `Result: OK`, og målte blant annet `Document: 990`, `SourceDoc: 193`, `Company: 185`, `CompanyOwnership: 75` og `Deliverable: 12`; samme run hadde ingen Node.js 20-warning. Dette er bare read-only B1-preflight; ingen prod-write-target (`ownership`, `registers` eller `full`) er kjørt, og PR #159-stackens nye `/api/data-status`-dekning er fortsatt ikke på prod. Beslutningspakken er også lagt som PR-kommentar slik at Gabriel kan svare direkte på G-06, G-10 og G-11 i reviewflaten.

## Kopierbar PR-review

Bruk ett av disse svarene i PR #159. Ikke flytt teksten til `decision-log-food-tg.md` før reviewkommentaren eller vedtaket faktisk foreligger.

Hvis alle tre beslutningspunkter godkjennes:
...
```text
G-06: endres for merge: [kort endring eller "ingen endring"]
G-10: endres for merge: [kort endring eller "ingen endring"]
G-11: endres for merge: [kort endring eller "ingen endring"]

PR #159 skal forbli draft til endringen er implementert, verifisert og lagt tilbake til review.
```
...
Status: Integrasjonsstacken er bygget og testet lokalt, og draft-PR #159 er teknisk mergebar med grønn GitHub CI etter seneste status-sync. PR #159 er live kilde for siste head/check-status. PR #182 er landet på `main`; JT uke 25-pakken har egen QA-guard i `tests/lib/jt-uke25-package.test.ts`. PR #175 er landet på `main` og gjør prod-data-import-workflowen klar for eksplisitt `verify-only`, `ownership`, `registers` og `full`-valg. PR #177 er landet på `main` og gjør A4-verifikasjonen kjørbar som `npm run verify:platform-stack-main`. PR #179 er landet på `main`, Coolify SHA Sync er grønn, og live `/api/version` er fasit for gjeldende deploy-SHA. PR #185 er landet på `main` med Node 24-kompatible GitHub Actions-pins (`actions/checkout@v6`, `actions/setup-node@v6`). prod-data-import `verify-only` run 27369814363 er grønn på `main`, med autentisert prod-DB-tunnel, `db:verify` OK og ingen Node.js 20-warning. PR #159 skal likevel ikke tas ut av draft før Gabriel har godkjent eller endret G-06, G-10 og G-11. Gjenstående strict source-rødt er klassifisert som baseline/operator-dataavvik, ikke som PR-spesifikk kodefeil.

Grønt lokalt etter seneste status-sync av PR #159:
- `npm ci` passerte med 0 vulnerabilities.
- `npm run db:generate` passerte.
- `npm test` passerte; PR #159-kommentaren er live kilde for siste test-/suite-tall.
