---
tags: ""
rute: /mandat
type: seksjon
status: generert
kilde: scripts/obsidian-vault/sync.ts
siterbarhet: intern
---

# Mandat

> Klynge: [[Klynge – Intern]] · Rute i appen: `/mandat`

Food TG scope, claims og validering — hva prosjektet har lov til å hevde.

## Datagrunnlag i repoet

- `research/rammeverk/leveranseplan-wp3-food-systems.md` — leveransestrategi WP3
- Kontrakt 201-2503-P25013: `research/external/nch-contract/contract-201-2503-P25013.md`
- `.claude/source-attribution-policy.md` — kildepolicy (claim-lock, gate)

## Relaterte seksjoner

- [[Team]]
- [[Kilder]]
- [[Hvitbok]]
## Notater

_Utvikles gjennom prosjektet._
