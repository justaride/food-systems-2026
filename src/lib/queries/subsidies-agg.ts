...
export async function getSubsidiesBySchemeAndStage(subsidyType = 'produksjonstilskudd') {
  try {
    ...
    if (isPrismaDataUnavailable(error)) return { schemes: [], stages: [], cells: [], maxNok: 0 }
    throw error
  }
}
...
export async function getSubsidyStageCoverage(subsidyType = 'produksjonstilskudd') {
  try {
    ...
    if (isPrismaDataUnavailable(error)) return { subsidyType, totalRecipients: 0, totalRows: 0, totalNok: 0, stages: [] }
    throw error
  }
}
...
export async function getSubsidyTotals() {
  try {
    ...
    if (isPrismaDataUnavailable(error)) return { totalNok: 0, totalRows: 0, byType: [] }
    throw error
  }
}
