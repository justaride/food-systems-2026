...
import type { SourceRow } from '@/app/kilder/KilderContent'
...
export async function getSourceCatalog() {
  ...
  return { sources: allSources, rounds: roundOptions, dbCount: enrichedFromDb.length,
    documentCount: enrichedFromDocuments.length, backlogOnlyCount: backlogOnly.length }
}
