...
export default function KilderPage() {
  const [filter, setFilter] = useState<SourceType | 'alle'>('alle')
  const [search, setSearch] = useState('')

  const filteredSources = sources.filter(src => {
    const matchesFilter = filter === 'alle' || src.type === filter
    const matchesSearch = 
  ...
  const stats = {
    ...
    nou: sources.filter(s => s.type === 'nou').length,
    rapport: sources.filter(s => s.type === 'rapport').length,
    analyse: sources.filter(s => s.type === 'analyse').length,
    lovverk: sources.filter(s => s.type === 'lovverk').length,
  }
  ...
  return (
    <div className="space-y-8 max-w-7xl mx-auto pb-12">
      ...
      <div className="flex flex-col md:flex-row gap-4 items-center bg-stone-50 p-4 rounded-xl border border-stone-200">
        ...
        <div className="flex flex-wrap gap-2">
          {Object.entries(typeLabels).map(([key, label]) => {
            const count = key === 'alle' ? sources.length : sources.filter(s => s.type === key).length
      ...
      {/* Grid Visning for tunge kilder (NOU/Analyser) */}
