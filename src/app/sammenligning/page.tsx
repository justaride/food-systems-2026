...
function getSelfSufficiency(code: CountryCode): number {
  const entries = countryChartData[code]?.selfSufficiency?.data ?? []
...
export default function SammenligningPage() {
  ...
  const selfSuffData = countries.map(c => ({
    country: c.name,
    flag: c.flag,
    value: getSelfSufficiency(c.code),
    label: `${getSelfSufficiency(c.code)}%`,
  }))
