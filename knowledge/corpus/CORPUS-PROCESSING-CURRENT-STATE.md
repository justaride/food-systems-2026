...
The reviewable root-writer v1.2.1 source fixes the production ledger root, a separate root-only request-intake root and an administrator-pinned request-file hash. Its successor rules require the prior trusted-verification hash and immutable baseline/register/generation bindings. It has not been installed or run, and no external ledger exists. The repository-side genesis trusted-verification appender, transactional pending-event writer and post-external event-checkpoint finalizer are implemented. They form a two-phase workflow in which the first phase remains deliberately unavailable to normal consumers and the second phase publishes only the externally verified successor through a five-file atomic transaction. Copying a verification record or projection by hand is not an accepted workflow.
