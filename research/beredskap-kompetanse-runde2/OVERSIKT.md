...
| `pastander-samlet.csv` | De 156 påstandene fra CSV-blokkene i R1–R6 (R1 42, R2 22, R3 18, R4 25, R5 29, R6 20), med kolonnene `prompt` og `kildefil` lagt til. `status` finnes bare for R5 og `niva` (bruksnivå 1–5) bare for R6; feltene er tomme ellers. |
| `kontroll/R1–R7-kontroll.md`, `kontroll/X-kontroll.md` | Første kontroll per rapport |
...
| R7 | 20 | 10 | 0 | 3 | 0 |
| X | 24 | 10 | 0 | 1 | 0 |
| **Første runde** | **181** | **88** | **1** | **5** | **0** |
| **Etter andre runde** | **184** | **90** | **1** | **0** | **0** |
...
Oppdatert prompt for R8 (og R5–R7), klar til kjøring: [RUNDE2-R5-R8-PROMPTER.md](../../docs/project/analysis/beredskap-kompetanse-2026-09-14/RUNDE2-R5-R8-PROMPTER.md).

## Grenser

- Kontrollen er KI-basert. Den er ikke faglig godkjenning.
- Høyst ca. 35 påstander per rapport er kontrollert. R1 har sju fraværsfunn uten kilde som ikke er kontrollert. Fraværsfunn i R5–R7 er heller ikke kontrollert.
