...
async function importFinancials() {
  ...
  for (const f of data) {
    ...
    await prisma.companyFinancial.upsert({
      ...
      update: { ...wholeNokStorage, revenueNok: f.revenueNok, operatingResult: f.operatingResult ?? null, operatingMargin: f.operatingMargin, groupEmployees: f.groupEmployees, source: 'Broennoysund / offentligdata MCP 2026-03' },
      create: { ...wholeNokStorage, companyId, year: f.year, revenueNok: f.revenueNok, operatingResult: f.operatingResult ?? null, operatingMargin: f.operatingMargin, groupEmployees: f.groupEmployees, source: 'Broennoysund / offentligdata MCP 2026-03' },
    })
