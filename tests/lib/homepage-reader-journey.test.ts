...
describe('homepage reader journey', () => {
  it('prioritizes current work and project status before the illustrative atlas without ambiguous hero KPIs', () => {
    assert.match(pageSource, /import \{ MatsystemetsSnitt \}/)
    assert.equal((pageSource.match(/<MatsystemetsSnitt \/>/g) ?? []).length, 1)
    assert.ok(pageSource.indexOf('aria-labelledby="current-work-title"') < pageSource.indexOf('Aktiv fase'))
    assert.ok(pageSource.indexOf('Aktiv fase') < pageSource.indexOf('<MatsystemetsSnitt />'))
