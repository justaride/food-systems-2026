...
test("binds analysis and validation to different workflow and prompt bytes", () => {
  ...
  assert.equal(analysis.workflow.version, "1.0.19");
  assert.equal(analysis.prompt.version, "1.0.19");
  assert.equal(validation.workflow.version, "1.0.19");
  assert.equal(validation.prompt.version, "1.0.19");
