-- Advance the sealed library analysis and validation bindings to 1.0.18
    ...
    IF run_row.id IS NULL OR run_row.id !~ '^[a-z0-9][a-z0-9._:-]*$'
      ...
      OR NOT (
        (
          ...
          AND run_row."workflowVersion" = '1.0.18'
          AND run_row."workflowPath" = 'knowledge/corpus/workflows/library-analysis-automated-v1.md'
          AND run_row."workflowHash" = '6aff71f079426395122f050a0d279a83a9fb6ac1354251cccb552ac04add3de8'
          AND run_row."promptId" = 'prompt.library_analysis.automated.v1'
          AND run_row."promptVersion" = '1.0.18'
          AND run_row."promptPath" = 'knowledge/corpus/workflows/library-analysis-automated-prompt-v1.md'
          AND run_row."promptHash" = 'fc2e28009fee4ca08ddb491d69ab137acbcd8cd26e93e61ef1353d6b060653d3'
        ) OR (
          ...
          AND run_row."workflowHash" = '2e27617bd3c9710124b0c4660c58adb1773115fa903622a77d64cc49b7034dcd'
          ...
          AND run_row."promptHash" = 'b2ef6f9b425a9f5fc6124308fe940535474c8b7d079d332fb127bcee529cb44a'
        )
      )
