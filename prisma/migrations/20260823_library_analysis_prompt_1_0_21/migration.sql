-- Advance the sealed library analysis and validation bindings to 1.0.21
    ...
    IF run_row.id IS NULL OR run_row.id !~ '^[a-z0-9][a-z0-9._:-]*$'
      ...
      OR NOT (
        (
          ...
          AND run_row."workflowVersion" = '1.0.21'
          AND run_row."workflowPath" = 'knowledge/corpus/workflows/library-analysis-automated-v1.md'
          AND run_row."workflowHash" = '6841d0a7b9fcd2219b3ed3be1192bfd660d9f9db68e62a248a1821d29ea3b203'
          AND run_row."promptId" = 'prompt.library_analysis.automated.v1'
          AND run_row."promptVersion" = '1.0.21'
          AND run_row."promptPath" = 'knowledge/corpus/workflows/library-analysis-automated-prompt-v1.md'
          AND run_row."promptHash" = '4fd2e46f21a02c0db6054dbb91eaa8b3a2a0e2979799872226cd3e34281f0566'
        ) OR (
          ...
          AND run_row."workflowHash" = 'c3daec89bc61aa23df0c0aa03d675ebc3d92920c357dd751df099cf6d5a40d6b'
          ...
          AND run_row."promptHash" = 'c04f8505c8728371b080b4c15272e4de64e3506d00a94ff33e10c25058a0e2af'
        )
      )
